...
public class UriBuilderResource {
	...
	@GET
	@Path("/test5")
	// http://localhost:8080/injavawetrust.resteasy.tutorial/uriBuilder-path/test5
	public String test5UriBuilder() {

		UriBuilder builder = UriBuilder.fromPath("/customers/{id}");
		URI uri = builder.build("a/b");
		return uri.toString();
	...
	@GET
	@Path("/test6")
	// http://localhost:8080/injavawetrust.resteasy.tutorial/uriBuilder-path/test6
	public String test6UriBuilder() {

		UriBuilder builder = UriBuilder.fromPath("/customers/{id}");
		URI uri = builder.resolveTemplate("id", "a/b", false).build();
		return uri.toString();

		// public abstract UriBuilder resolveTemplate(String name, Object value, boolean
		// encodeSlashInPath);
